// src/App.jsx — dark+gold, zentriert, Dashboard, Icons, build-fähig
...
/* ========================= ICONS (inline SVG, gold) ========================= */
const IconWrap = ({ children }) => (
  <div className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-[#1d1d1d] border border-[#2a2a2a] mr-2">
    {children}
  </div>
);

const Gold = ({ stroke = 1.8 }) => ({ fill: "none", stroke: theme.gold, strokeWidth: stroke, strokeLinecap: "round", strokeLinejoin: "round" });

const IconPrice = () => (
  <svg viewBox="0 0 24 24" width="18" height="18" {...Gold({})}>
    <path d="M12 3v18M7 7h7.5a3.5 3.5 0 0 1 0 7H9" />
  </svg>
);
const IconMessage = () => (
  <svg viewBox="0 0 24 24" width="18" height="18" {...Gold({})}>
...
/* ========================= PRIMITIVES ========================= */
function Card({ title, subtitle, icon = null, className = "", children }) {
  return (
    <div className={`rounded-xl p-5 transition-all duration-300 ${theme.card} ${theme.cardHover} ${className}`}>
      {(title || subtitle || icon) && (
        <div className="mb-3">
...
function Button({ children, onClick, variant = "solid", className = "", type = "button", disabled = false }) {
  // ⚠️ Kein Backtick-Template in Klassen! Nur statische Klassen + CSS-Variablen.
  const styleVars = { "--gold": theme.gold, "--goldHover": theme.goldHover };
  const baseSolid = "hover:bg-[var(--goldHover)]";
  const baseOutline = "border border-[var(--gold)] text-[var(--gold)] hover:bg-[var(--gold)] hover:text-black";
  ...
  return (
    <button
      ...
      className={`rounded-2xl px-5 py-2 text-sm font-semibold transition-all duration-200 disabled:opacity-50 ${base} ${className} hover:-translate-y-0.5 active:translate-y-0`}
    >
      {children}
    </button>
  );
      ...
      <nav className="flex flex-wrap items-center justify-center gap-2">
        {session ? (
          <>
            ...
            {me?.role === "admin" && <Button variant="outline" onClick={() => { setTool(null); setView("admin"); }}>Admin</Button>}
    ...
    <div className={`min-h-screen ${theme.bg} ${theme.text}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
        ...
        <main className="space-y-10">
            ...
            <FadeIn inKey={tool}>
              <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-6">
                ...
                <section className="lg:col-span-8 space-y-4">
                  {tool === "pricefinder" && (
                    <Card title="PriceFinder" icon={<IconPrice />}>
                      <PriceFinder />
                    </Card>
                  )}
                  {tool === "messagematcher" && (
                    <Card title="MessageMatcher" icon={<IconMessage />}>
                      <MessageMatcher />
                    </Card>
                  )}
                  {tool === "contentflow" && (
                    <Card title="ContentFlow" icon={<IconContent />}>
                      <ContentFlow />
                    </Card>
                  )}
                </section>
          ...
          {/* AUTH / ACCOUNT */}
          {!session && view === "login" && <FadeIn inKey="login"><LoginView setBanner={setBanner} setView={setView} /></FadeIn>}
          {!session && view === "register" && <FadeIn inKey="register"><RegisterView setBanner={setBanner} setView={setView} /></FadeIn>}
          {view === "recovery" && <FadeIn inKey="recovery"><RecoveryView setBanner={setBanner} /></FadeIn>}
          {session && view === "account" && !tool && <FadeIn inKey="account"><AccountView me={me} setBanner={setBanner} /></FadeIn>}
          {session && view === "admin" && !tool && <FadeIn inKey="admin"><Admin onBack={() => setView("home")} /></FadeIn>}
    ...
    try {
      ...
      if (uid) {
        const { error: pe } = await supabase
          .from("profiles")
          .upsert({ id: uid, email: email.trim().toLowerCase(), role: "user", first_name: first || null });
        if (pe) throw pe;
      }
